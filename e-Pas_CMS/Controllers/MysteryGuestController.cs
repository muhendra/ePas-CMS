...
{
    public class MysteryGuestController : Controller
    {
        private readonly EpasDbContext _context;
        private const int DefaultPageSize = 10;
        private readonly ILogger<MysteryGuestController> _logger;
        private readonly IConfiguration _configuration;

        public MysteryGuestController(EpasDbContext context, ILogger<MysteryGuestController> logger, IConfiguration configuration)
        {
            _context = context;
            _logger = logger;
        ...
        public async Task<IActionResult> Index(int pageNumber = 1, int pageSize = DefaultPageSize, string searchTerm = "", string sortColumn = "TanggalAudit", string sortDirection = "desc")
        {
            try
            {
                ...
                var query = from a in _context.trx_audits
                            ...
                            where a.status == "UNDER_REVIEW" && a.audit_type == "Mystery Guest"
                            select new
                            {
                                Audit = a,
                                Spbu = s,
        ...
        [HttpPost("MysteryGuest/approve/{id}")]
        ...
        public async Task<IActionResult> GenerateAllVerifiedPdfBoa()
        {
            ...
            try
            {
                ...
                foreach (var id in auditIds)
                {
                    ...
                    "UPDATE trx_audit SET report_file_mg = {0} WHERE id = {1}",
    ...
    WHERE ta.id = @id";
        ...
        [HttpPost("MysteryGuest/UploadBeritaAcaraMedia")]
    ...
    (uuid_generate_v4(), @auditId, null, @mediaType, @mediaPath, 'FINAL', 'ACTIVE', NOW(), @createdBy, NOW(), @createdBy)";
        ...
        public async Task GenerateAllVerifiedPdfReports(string ids)
        {
            ...
            try
            {
                ...
                var auditIds = await _context.trx_audits
                    .Where(a => a.status == "VERIFIED" && a.audit_type != "Mystery Guest" && a.report_file_excellent == null && a.id == ids)
                    .OrderByDescending(a => a.audit_execution_time)
                    //.Take(10)
                    .Select(a => a.id)
                    .ToListAsync(token);  // <- inject token
...
AND mqd.type = 'QUESTION'";
        ...
        [HttpPost("MysteryGuest/UploadDocument")]
...
VALUES 
    (uuid_generate_v4(), @auditId, @nodeId, @mediaType, @mediaPath, 'QUESTION', 'ACTIVE', NOW(), @createdBy, NOW(), @createdBy)";
        ...
        [HttpGet("MysteryGuest/ViewDocument/{auditId}/{id}")]
