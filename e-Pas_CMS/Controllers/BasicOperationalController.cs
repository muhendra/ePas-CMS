...
namespace e_Pas_CMS.Controllers
{
    ...
    public class BasicOperationalController : Controller
    {
        ...
        public BasicOperationalController(EpasDbContext context, ILogger<BasicOperationalController> logger, IConfiguration configuration)
        {
            _context = context;
            _logger = logger;
        ...
        public async Task<IActionResult> Index(int pageNumber = 1, int pageSize = DefaultPageSize, string searchTerm = "", string sortColumn = "TanggalAudit", string sortDirection = "desc")
        {
            try
            {
                ...
                var checklistByAudit = await GetBasicOperationalIndexChecklistDataAsync(conn, auditIds);
                var penaltyFlagsByAudit = await GetBasicOperationalIndexPenaltyFlagsAsync(conn, auditIds);
                ...
                foreach (var a in items)
                {
                    var checklistData = checklistByAudit.TryGetValue(a.Audit.id, out var auditChecklist)
                        ? auditChecklist
                        : new List<BasicOperationalIndexChecklistRow>();
                    var questionRows = checklistData
                        .Where(x => string.Equals(x.type, "QUESTION", StringComparison.OrdinalIgnoreCase))
                        .ToList();

                    decimal finalScore = CalculateBasicOperationalIndexFinalScore(questionRows);
        ...
        private sealed class BasicOperationalIndexChecklistRow : ChecklistFlatItem
        {
            public string TrxAuditId { get; set; }
        }

        private async Task<Dictionary<string, List<BasicOperationalIndexChecklistRow>>> GetBasicOperationalIndexChecklistDataAsync(
            IDbConnection conn,
            string[] auditIds)
        {
            if (auditIds.Length == 0)
                return new Dictionary<string, List<BasicOperationalIndexChecklistRow>>(StringComparer.OrdinalIgnoreCase);
            ...
            var rows = (await conn.QueryAsync<BasicOperationalIndexChecklistRow>(sql, new { auditIds })).ToList();
        ...
        private async Task<Dictionary<string, bool>> GetBasicOperationalIndexPenaltyFlagsAsync(
            IDbConnection conn,
            string[] auditIds)
        {
            if (auditIds.Length == 0)
        ...
        private static decimal CalculateBasicOperationalIndexFinalScore(IEnumerable<BasicOperationalIndexChecklistRow> questionRows)
        {
            ...
            foreach (var item in questionRows)
            {
                ...
                string input = NormalizeBasicOperationalIndexScore(item.score_input);
        ...
        private static string NormalizeBasicOperationalIndexScore(string score)
        {
        ...
        public async Task<IActionResult> Approve(string id)
        {
            ...
            await GenerateAllVerifiedPdfBoa();
